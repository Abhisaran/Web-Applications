                        ...
                        <?php
                            ...
                            $result = mysqli_query($conn,$sql);
                                ...
                                function dateDiff($start,$end)
                                {
                                    ...
                                    if(round($diff/86400)<0 && $end =="0000-00-00 00:00:00")
                                        return "Never";
                                    else if(abs(round($diff/3600))<24)
                                        return abs(round($diff/3600))." hr(s) ago";
                                    else if(abs(round($diff/86400))<31)
                                        return (abs(round($diff/86400)))." day(s) ago";
                                    else return (abs(round($diff/2678400)))." month(s) ago";
                                                
                                }
                                
                            while($row = mysqli_fetch_assoc($result)){
                                echo "<center><table class='table table-striped'><tr><td>";
                                echo "Username:</td><td>".ucfirst($row["username"])."</td</tr>";
                                echo "<tr><td>Account type:</td><td>".ucfirst($row["type"])."</td></tr>";
                                echo "<tr><td>Last password changed:</td><td>".dateDiff(date('Y-m-d H:i:s'),$row["lachge"])."</td></tr>";                                
                                echo "</table>";
                                echo "Change Password <a href='change.html'>here</a>";
